...
import { callLLM, render, Skill, UseSkillTool, resolveSkills, md } from "../src"
...
const SetObjectivesTool = () => (
    <tool name="set_objectives" description="Define or update a numbered list of implementation objectives before coding.">
        <param name="objectives" type="string" required>A numbered list of concrete objectives</param>
        <param name="reasoning" type="string" required>Why these objectives were chosen</param>
    </tool>
)

const WriteFileTool = () => (
    <tool name="write_file" description="Write a file, creating directories if needed">
...
const DoneTool = () => (
    <tool name="done" description="Signal that the plan is complete">
        <param name="summary" type="string" required>Summary of what was completed</param>
    </tool>
)

function buildPrompt(messages: Array<{ role: "user" | "assistant"; content: string }>, resolvedSkillNames: string[] = []) {
...
function summarizeToolCalls(toolCalls: Array<{ name: string; args: Record<string, any> }>) {
...
function requestedSkillsFrom(result: Awaited<ReturnType<typeof callLLM>>): string[] {
...
const userRequest = process.argv[2] || "Build a tiny Bun HTTP JSON API with tests"
...
if (turn1.usage) console.log(`Tokens: ${turn1.usage.inputTokens} in → ${turn1.usage.outputTokens} out`)
console.log()
...
const turn2Prompt = buildPrompt([
    ...
    {
        role: "user",
        content: "Continue with the requested skills now resolved. Propose the first concrete implementation actions.",
    },
], requestedSkills)

const turn2Extracted = render(turn2Prompt)
console.log("Turn 2 (resolved)")
console.log(`- resolved skills embedded: ${resolved.map(s => s.name).join(", ")}`)
...
if (turn2.usage) console.log(`Tokens: ${turn2.usage.inputTokens} in → ${turn2.usage.outputTokens} out`)
