...
function appServerInput(
  ...
  if (items.length === 0) {
    throw new JsxAiError(
      "INVALID_ARGUMENT",
      "Codex app-server turn input must not be empty",
    );
  }
  return items.map((item, index) => {
    if (item.type === "text") {
      if (!item.text)
        throw new JsxAiError(
          "INVALID_ARGUMENT",
          `Codex turn input[${index}] text must not be empty`,
        );
      return { type: "text", text: item.text } as JsonObject;
    }
    if (!item.path)
      throw new JsxAiError(
        "INVALID_ARGUMENT",
        `Codex turn input[${index}] localImage path must not be empty`,
      );
    return { type: "localImage", path: item.path } as JsonObject;
  });
}
...
function usageFromNotification(
  ...
  if (!matchingIds(params, threadId, turnId) || !isRecord(params.tokenUsage))
    return undefined;
    ...
    try {
      ...
      while (true) {
        ...
        if (
          ...
          const delta =
            isRecord(event.params) && typeof event.params.delta === "string"
              ? event.params.delta
              : "";
          if (delta) {
            deltaText += delta;
        ...
        if (event.method === "thread/tokenUsage/updated") {
          usage =
            usageFromNotification(event.params, threadId, turnId) ?? usage;
          continue;
        }
        ...
        if (event.method === "item/completed") {
          ...
          finalResponse =
            completedAgentText(event.params, threadId, turnId) ?? finalResponse;
        }
        ...
        if (
          ...
          if (isRecord(event.params) && event.params.willRetry === true)
            continue;
          const message =
    ...
    const turn = this.runTurn(threadId, input, runtimeOptions, {
      ...
      (result) => {
        ...
        if (!streamedText && result.finalResponse)
          queue.push(result.finalResponse);
        queue.close();
        return result;
      },
      (error) => {
