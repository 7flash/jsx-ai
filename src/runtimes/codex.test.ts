...
describe("Codex unified app-server runtime", () => {
  ...
  it("feeds tool-result image attachments into the next Codex turn without resending old images", async () => {
    const capture: LaunchCapture = { children: [] };
    process.env.JSX_AI_RUNTIME = "codex";
    installFakeServer(
      [
        ...
        {
          response: JSON.stringify({
            text: "I saw the screenshot; I need one more text observation.",
            toolCalls: [{ name: "list_files", arguments_json: "{}" }],
          }),
        },
        {
          response: JSON.stringify({
            text: "I can see the running game.",
            toolCalls: [
              {
                name: "done",
                arguments_json: JSON.stringify({ summary: "seen" }),
              },
            ],
          }),
        },
    ...
    const imagePath = ".agent/screenshots/game.png";
    let listCalls = 0;
    const result = await runAgent({
      history: [{ role: "user", content: "Inspect the game" }],
      buildPrompt: (history) => agentPrompt(history),
      callOptions: {
        codex: {
          codexPathOverride: "fake-codex",
      ...
      executeTool: (call) => {
        if (call.name !== "list_files") return "done";
        listCalls++;
        return listCalls === 1
          ? {
              content: "Current game screenshot.",
              attachments: [
          ...
          : {
              content:
                "No new screenshot; continue from the previous visual observation.",
            };
      },
      isComplete: (response) =>
        response.toolCalls.some((call) => call.name === "done"),
    ...
    expect(result.reason).toBe("completed");
    const turns =
      capture.children[0]?.requests.filter(
        (request) => request.method === "turn/start",
      ) ?? [];
    expect(turns).toHaveLength(3);
    const firstInput =
      (turns[0]?.params as { input?: unknown[] } | undefined)?.input ?? [];
    const secondInput =
      (turns[1]?.params as { input?: unknown[] } | undefined)?.input ?? [];
    const thirdInput =
      (turns[2]?.params as { input?: unknown[] } | undefined)?.input ?? [];
    expect(firstInput).toHaveLength(1);
    expect(secondInput).toContainEqual({
      type: "localImage",
      path: resolvePath(process.cwd(), imagePath),
    });
    expect(thirdInput).toHaveLength(1);
    expect(result.steps[1]?.response.request?.body.bridgeAttachmentsSent).toBe(
      1,
    );
    expect(result.steps[1]?.response.request?.body.bridgeAttachmentsTotal).toBe(
      1,
    );
    expect(result.steps[2]?.response.request?.body.bridgeAttachmentsSent).toBe(
      0,
    );
    expect(result.steps[2]?.response.request?.body.bridgeAttachmentsTotal).toBe(
      1,
    );
  });
    ...
    const result = await runAgent({
      ...
      onEvent: (event) => {
        ...
        if (event.type === "tool_progress") {
          ...
          if (tool.type === "tool_detected")
            progress.push(`tool_detected:${tool.name}`);
          if (tool.type === "field_delta" && tool.path[0] === "content") {
            progress.push(`content+${tool.delta.length}`);
          }
          if (tool.type === "field_ready")
            progress.push(`field_ready:${tool.path.join(".")}`);
          if (tool.type === "tool_ready")
            progress.push(`tool_ready:${tool.call.name}`);
        }
      },
    });
