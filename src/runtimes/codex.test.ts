...
class FakeAppServerProcess extends EventEmitter {
    ...
    private handleRequest(line: string): void {
        ...
        if (message.method !== "turn/start" || typeof message.id !== "number") return
        ...
        const config = this.options.turns[this.turn - 1] ?? this.options.turns.at(-1)
        if (!config) throw new Error(`No fake turn configured for turn ${this.turn}`)
        ...
        queueMicrotask(() => {
            ...
            if (config.progress) {
                this.send({
                    ...
                    params: { threadId, turnId, itemId: `reason_${this.turn}`, delta: config.progress },
                })
            }
            if (config.usage) {
                    ...
                    params: {
                        ...
                        tokenUsage: {
                            last: {
                                ...
                                totalTokens: config.usage.totalTokens ?? inputTokens + outputTokens,
                            },
                            total: {},
            ...
            if (config.error) {
                this.send({
                    method: "error",
                    params: { threadId, turnId, willRetry: false, error: { message: config.error } },
                })
                return
            }

            const response = config.response ?? JSON.stringify({ text: "ok", toolCalls: [] })
            ...
            this.send({
                ...
                params: {
                    threadId,
                    turnId,
                    item: { id: `message_${this.turn}`, type: "agentMessage", text: response },
                },
            })
            this.send({
...
function installFakeServer(turns: readonly FakeTurn[], capture: LaunchCapture): void {
...
function agentPrompt(history: readonly ExtractedMessage[]) {
    return Fragment({
        children: [
            jsx("system", { children: "Use host tools to inspect and finish the task." }),
            jsx("tool", { name: "list_files", description: "List project files" }),
            jsx("tool", {
                name: "done",
                description: "Finish the task",
                children: jsx("param", { name: "summary", type: "string", required: true }),
            }),
            ...history.map(message => jsx("message", {
                role: message.role,
...
describe("Codex unified app-server runtime", () => {
    it("round-trips canonical tools, usage, auth environment, and structured output", async () => {
        ...
        installFakeServer([{
            response: JSON.stringify({
                ...
                toolCalls: [{
                    name: "write_file",
                    arguments_json: JSON.stringify({ path: "index.html", content: "<canvas></canvas>" }),
                }],
            }),
            usage: { inputTokens: 120, cachedInputTokens: 80, outputTokens: 25, reasoningOutputTokens: 7 },
        }], capture)
        ...
        expect(result.toolCalls[0]?.args).toEqual({ path: "index.html", content: "<canvas></canvas>" })
        expect(result.toolCalls[0]?.id).toContain("thread_1")
        expect(result.usage).toEqual({ inputTokens: 120, outputTokens: 25, thinkingTokens: 7 })
        expect(result.request?.url).toBe("codex://app-server")
        expect(result.raw).toMatchObject({ runtime: "codex", transport: "app-server" })
    ...
    it("can intentionally inherit API-key environment", async () => {
        ...
        installFakeServer([{ response: JSON.stringify({ text: "ok", toolCalls: [] }) }], capture)
        const codex: CodexRuntimeOptions = { auth: "inherit", codexPathOverride: "fake-codex" }
    ...
    it("rejects explicit apiKey so billing mode cannot change silently", async () => {
        await expect(callCodexRuntime(prompt(), undefined, {
            apiKey: "sk-explicit",
            codex: { codexPathOverride: "fake-codex" },
    ...
    it("does not add an authentication hint to unrelated runtime errors", async () => {
        ...
        try {
            await callCodexRuntime(prompt(), undefined, { codex: { codexPathOverride: "fake-codex" } })
            throw new Error("expected runtime failure")
        } catch (error) {
            expect(isJsxAiError(error, "RUNTIME_ERROR")).toBe(true)
            expect(error instanceof Error ? error.message : String(error)).toContain("worker crashed")
            expect(error instanceof Error ? error.message : String(error)).not.toContain("codex login")
        }
    })
    ...
    it("routes callLLM through Codex from environment without requiring an API key or model", async () => {
        ...
        installFakeServer([{ response: JSON.stringify({ text: "from env", toolCalls: [] }) }], capture)

        const result = await callLLM(jsx("message", { role: "user", children: "Say hello" }), {
            codex: { codexPathOverride: "fake-codex" },
        })

        expect(result.text).toBe("from env")
        expect(result.request?.url).toBe("codex://app-server")
        const threadStart = capture.children[0]?.requests.find(request => request.method === "thread/start")
        expect(threadStart?.params).not.toMatchObject({ model: expect.any(String) })
    })
    ...
    it("forwards progress, reuses one thread within runAgent, sends deltas, and closes the process", async () => {
        ...
        installFakeServer([
            {
                ...
                usage: { inputTokens: 100, cachedInputTokens: 40, outputTokens: 10, reasoningOutputTokens: 2 },
            },
            {
                response: JSON.stringify({
                    text: "The project is ready.",
                    toolCalls: [{ name: "done", arguments_json: JSON.stringify({ summary: "complete" }) }],
                }),
                usage: { inputTokens: 130, cachedInputTokens: 90, outputTokens: 12, reasoningOutputTokens: 1 },
            },
        ], capture)
        ...
        const result = await runAgent({
            ...
            executeTool: call => call.name === "list_files" ? "[\"index.html\"]" : "done",
            isComplete: response => response.toolCalls.some(call => call.name === "done"),
            onEvent: event => {
                if (event.type === "runtime_progress") progress.push(`${event.progress.kind}:${event.progress.message}`)
            },
            maxSteps: 3,
        })
        ...
        expect(progress.some(message => message.includes("Inspecting the project structure"))).toBe(true)
        expect(capture.children[0]?.exitCode).toBe(0)
    })

    it("streams assistant text and semantic tool construction while execution stays atomic", async () => {
        const capture: LaunchCapture = { children: [] }
        process.env.JSX_AI_RUNTIME = "codex"
        const response = JSON.stringify({
            ...
            toolCalls: [{
                ...
                arguments_json: JSON.stringify({
                    path: "game.js",
                    content: "SECRET_TOOL_PAYLOAD_123456789",
                }),
            }],
        })
        installFakeServer([{
            ...
            deltas: [
                response.slice(0, 21),
                response.slice(21, 57),
                response.slice(57, 93),
                response.slice(93),
            ],
        }], capture)
        ...
        const result = await runAgent({
            ...
            callOptions: { codex: { codexPathOverride: "fake-codex" } },
            executeTool: () => {
                expect(progress.at(-1)).toBe("tool_ready:list_files")
                executed = true
                return "listed"
            },
            maxSteps: 1,
            onEvent: event => {
                ...
                if (event.type === "tool_progress") {
                    ...
                    const tool = event.progress
                    if (tool.type === "tool_detected") progress.push(`tool_detected:${tool.name}`)
                    if (tool.type === "field_delta" && tool.path[0] === "content") {
                        progress.push(`content+${tool.delta.length}`)
                    }
                    if (tool.type === "field_ready") progress.push(`field_ready:${tool.path.join(".")}`)
                    if (tool.type === "tool_ready") progress.push(`tool_ready:${tool.call.name}`)
                }
            },
        })
        ...
        expect(deltas.join("")).not.toContain("SECRET_TOOL_PAYLOAD")
        expect(progress[0]).toBe("tool_detected:list_files")
        expect(progress).toContain("field_ready:path")
        expect(progress.some(entry => entry.startsWith("content+"))).toBe(true)
        expect(progress).toContain("field_ready:content")
        expect(progress.at(-1)).toBe("tool_ready:list_files")
    ...
    it("scopes native Codex process/thread reuse to one runAgent invocation", async () => {
        const capture: LaunchCapture = { children: [] }
        process.env.JSX_AI_RUNTIME = "codex"
        installFakeServer([{ response: JSON.stringify({ text: "idle", toolCalls: [] }) }], capture)
