...
import type { RuntimeProgress, RuntimeToolProgress } from "./internal/agent-runtime"
import { normalizePromptIR, normalizeToolCall } from "./ir"
import type { CallOptions } from "./llm"
import type { CanonicalToolCall, ExtractedMessage, JsonValue, JsxAiNode, LLMResponse, ToolCall } from "./types"
...
export type AgentToolExecutorResult = AgentToolResult | ExtractedMessage | string
...
export type AgentEvent<State = unknown> =
    ...
    | { type: "runtime_progress"; context: AgentContext<State>; progress: AgentRuntimeProgress }
    | { type: "text_delta"; context: AgentContext<State>; delta: string }
    | { type: "tool_progress"; context: AgentContext<State>; progress: AgentToolProgress }
    | { type: "model_end"; context: AgentContext<State>; response: LLMResponse }
    | { type: "tool_start"; context: AgentContext<State>; call: CanonicalToolCall }
    | { type: "tool_end"; context: AgentContext<State>; call: CanonicalToolCall; result: ExtractedMessage }
...
export interface AgentRunOptions<State = undefined> {
    ...
    buildPrompt: (history: readonly ExtractedMessage[], context: AgentContext<State>) => JsxAiNode
     ...
     */
    onTextDelta?: (event: AgentTextDelta, context: AgentContext<State>) => void | Promise<void>
    /**
     ...
     */
    onToolProgress?: (event: AgentToolProgress, context: AgentContext<State>) => void | Promise<void>
    /**
...
function normalizeCalls(calls: readonly ToolCall[], step: number): CanonicalToolCall[] {
    return calls.map((call, index) => normalizeToolCall(
        call,
...
function normalizeToolResult(call: CanonicalToolCall, value: AgentToolExecutorResult): ExtractedMessage {
    if (typeof value === "string") {
        return Object.freeze({ role: "tool", content: value, toolCallId: call.id, toolName: call.name })
    }

    if ("role" in value) {
        if (value.role !== "tool") throw new Error(`Tool executor for ${call.name} returned a non-tool message`)
        if (value.toolCallId !== call.id) {
            throw new Error(`Tool executor for ${call.name} returned toolCallId ${JSON.stringify(value.toolCallId)}; expected ${JSON.stringify(call.id)}`)
        }
        if (value.toolName !== call.name) {
            throw new Error(`Tool executor for ${call.name} returned toolName ${JSON.stringify(value.toolName)}; expected ${JSON.stringify(call.name)}`)
        }
...
function budgetLimit(value: number | undefined): number {
    ...
    if (Number.isNaN(value) || value < 0) throw new Error(`Agent budget must be a non-negative number; received ${value}`)
    return value
}
 ...
 */
export async function runAgent<State = undefined>(options: AgentRunOptions<State>): Promise<AgentRunResult<State>> {
    const history = [...normalizePromptIR({ tools: [], messages: options.history ?? [] }).messages]
    const steps: AgentStep[] = []
    const usage: AgentUsage = { inputTokens: 0, outputTokens: 0, thinkingTokens: 0 }
    ...
    const externalSignal = combineSignals(options.signal, options.callOptions?.signal)
    ...
    const budgetStopReason = (): AgentStopReason | undefined => {
        ...
        if (deadlineSignal?.aborted || Date.now() - startedAt >= maxDurationMs) return "max_duration"
    ...
    const finish = async (reason: AgentStopReason, step: number): Promise<AgentRunResult<State>> => {
    ...
    try {
        for (let step = 0; step < maxSteps; step++) {
            ...
            const emitToolProgress = async (progress: RuntimeToolProgress) => {
                if (progress.type === "tool_detected") detectedTools.set(progress.index, progress.name)
                if (progress.type === "field_ready") readyFields.add(fieldProgressKey(progress.index, progress.path))
            ...
            try {
                ...
                runtimeContext.onProgress = progress => emit({
                    type: "runtime_progress",
                    context: context(step),
                    progress,
                })
                runtimeContext.onTextDelta = options.onTextDelta || options.onEvent
                    ? emitTextDelta
                    : undefined
                runtimeContext.onToolProgress = options.onToolProgress || options.onEvent
                    ? emitToolProgress
                    : undefined
                const callOptions = {
                    ...options.callOptions,
                ...
                rawResponse = await invoke(options.buildPrompt(history, context(step)), callOptions)
            } catch (error) {
                if (externalSignal?.aborted) return finish("aborted", step)
                if (deadlineSignal?.aborted || Date.now() - startedAt >= maxDurationMs) return finish("max_duration", step)
                throw error
            }
            ...
            if ((options.onTextDelta || options.onEvent) && response.text && !streamedText) {
            ...
            history.push(Object.freeze({ role: "assistant", content: response.text, toolCalls: Object.freeze(calls) }))
            await emit({ type: "model_end", context: context(step), response })

            if (!calls.length) {
                const recovery = await options.onNoToolCalls?.(response, context(step))
                steps.push({ index: step, response, toolResults: [], durationMs: Date.now() - stepStartedAt })
            ...
            if (toolCallsExecuted + calls.length > maxToolCalls) {
                const skippedResults = calls.map(call => normalizeToolResult(call, {
                    content: "Tool call was not executed because the agent tool-call budget was exhausted.",
                    isError: true,
                }))
                history.push(...skippedResults)
                steps.push({ index: step, response, toolResults: skippedResults, durationMs: Date.now() - stepStartedAt })
                return finish("max_tool_calls", step)
            }

            const executeOne = async (call: CanonicalToolCall): Promise<ExtractedMessage> => {
                if (operationSignal?.aborted) {
                    return normalizeToolResult(call, { content: "Agent run aborted before tool execution.", isError: true })
                }
                await emit({ type: "tool_start", context: context(step), call })
                let result: ExtractedMessage
                try {
                    result = normalizeToolResult(call, await options.executeTool(call, context(step)))
                } catch (error) {
