...
describe("runAgent", () => {
  ...
  test("keeps image attachments on canonical tool results", async () => {
    const result = await runAgent({
      buildPrompt: () => emptyTree,
      call: async () => response([{ name: "game_snapshot", args: {} }]),
      executeTool: () => ({
        content: "",
        attachments: [
          {
    ...
    expect(tool?.attachments?.[0]?.path).toBe(".agent/screenshots/game.png");
    expect(Object.isFrozen(tool?.attachments)).toBe(true);
    expect(Object.isFrozen(tool?.attachments?.[0])).toBe(true);
  });
  ...
  test("exposes semantic tool progress for buffered runtimes before execution", async () => {
    ...
    const result = await runAgent({
      ...
      isComplete: (model) =>
        model.toolCalls.some((call) => call.name === "write_file"),
      onToolProgress: (event) => {
        expect(executed).toBe(false);
        if (event.type === "tool_detected")
          progress.push(`tool_detected:${event.name}`);
        if (event.type === "field_ready")
          progress.push(`field_ready:${event.path.join(".")}`);
        if (event.type === "tool_ready")
          progress.push(`tool_ready:${event.call.name}`);
      },
    });
    ...
    const result = await runAgent({
      ...
      onEvent: async (event) => {
        ...
        if (event.type === "tool_progress") {
          ...
          if (event.progress.type === "tool_detected")
            order.push(`prepare:${event.progress.name}`);
          if (event.progress.type === "field_ready")
            order.push(`field:${event.progress.path.join(".")}`);
          if (event.progress.type === "tool_ready")
            order.push(`ready:${event.progress.call.name}`);
        }
        if (event.type === "model_end") order.push("model_end");
        if (event.type === "tool_start")
          order.push(`tool_start:${event.call.name}`);
        if (event.type === "tool_end")
          order.push(`tool_end:${event.call.name}`);
