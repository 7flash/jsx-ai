...
describe("jsx-runtime", () => {
  ...
  test("function components are called", () => {
    const MyTool = (props: any) =>
      h("tool", { name: props.name, description: props.desc });
...
describe("extract", () => {
  test("extracts a complete prompt", () => {
    ...
    expect(result.tools[0].parameters.properties.command.description).toBe(
      "The command to run",
    );
  ...
  test("extracts tools with multiple params", () => {
    ...
    expect(result.tools[0].parameters.required).toEqual([
      "path",
      "search",
      "replace",
    ]);
  ...
  test("extracts param with enum", () => {
    ...
    const result = extract(tree);
    expect(result.tools[0].parameters.properties.mode.enum).toEqual([
      "light",
      "dark",
      "auto",
    ]);
  ...
  test("handles fragment children", () => {
    ...
    const tree = h("prompt", {
      children: [tools, h("message", { role: "user", children: "Go" })],
    });
  ...
  test("extracts params nested inside fragments", () => {
    ...
    expect(result.tools[0].parameters.properties.path.description).toBe(
      "File path",
    );
    expect(result.tools[0].parameters.properties.search.description).toBe(
      "Text to find",
    );
  });
...
describe("native strategy", () => {
  ...
  test("parseResponse uses native FC tool calls from provider", () => {
    const result = native.parseResponse({
      ...
      nativeToolCalls: [{ name: "exec", args: { cmd: "ls -la" } }],
      raw: {},
